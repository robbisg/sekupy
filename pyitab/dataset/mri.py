...
import numpy as np
from pyitab.dataset.base import _expand_attribute
from pyitab.dataset.base import Dataset
from pyitab.dataset.mappers import FlattenMapper
...
def _hdr2dict(hdr):
    ...
    if scl_slope == scl_inter == 'nan':
        logger.warning("Detected incorrect (nan) scl_ fields. Resetting to "
...
def _img2data(src):
    ...
    if isinstance(img, nibabel.spatialimages.SpatialImage):
        ...
        if len(img.shape) == 5 and img.shape[3] == 1:
            ...
            logger.warning('dataset with 5th dimension found but 4th is empty (AFNI '
...
def map2nifti(dataset, data=None, imghdr=None, imgtype=None):
    """Map data(sets) into the original dataspace and wraps it into an Image.
    ...
    if imghdr is None:
        if 'imghdr' in dataset.a:
            ...
            logger.debug('No image header found. Using defaults.')
    ...
    if imgtype is None:
        if 'imgtype' in dataset.a:
            ...
            logger.debug('No image type found in %s. Using default Nifti1Image.',
                         dataset.a)
...
def _load_anyimg(src, ensure=False, enforce_dim=None):
    ...
    if (isinstance(src, list) or isinstance(src, tuple)) \
        ...
        # try opening the beast; this might yield none in case of an 
        # unsupported argument and is handled accordingly below
        data = _img2data(src)
        if data is not None:
    ...
    if imgdata is not None and enforce_dim is not None:
        ...
        if new_shape is not None:
            logger.debug('Enforcing shape %s for %s data from %s', 
                         new_shape, shape, src)
...
def strip_nibabel(ds):
    ...
    try:
        ...
        logger.debug('Failed to store header info as attribute (src: %s)', 
