...
class Atom(object):
    ...
    A `Atom` is an object the processes datasets. It can be called with a
    ...
    Atom have a notion of ``space``. The meaning of this space may vary heavily
    ...
    def __init__(self, space=None, pass_attr=None, postproc=None, **kwargs):
        ...
        postproc : Atom instance, optional
          Atom to perform post-processing of results. This node is applied
        ...
        self._calling_time = None
        self._raw_results = None

    def _get_call_kwargs(self, ds):
        """Provide _call kwargs, to be overriden in sub-classes.
    ...
    def __call__(self, ds, _call_kwargs={}):
        ...
        self._calling_time = time.time() - t0  # set the calling_time
        return result
    ...
    postproc = property(get_postproc, set_postproc,
                        doc="Atom to perform post-processing of results")
    
    
class CompoundAtom(Atom):
    """List of nodes.

    A CompoundAtom behaves similar to a list container: Atoms can be appended,
    and the chain can be sliced like a list, etc ...

    Subclasses such as ChainAtom and CombinedAtom implement the _call
...
class ChainAtom(CompoundAtom):
    ...
    chain. A ChainAtom may also be used as a generator. In this case, all
    nodes in the chain are treated as generators too, and the ChainAtom
    ...
    def __init__(self, nodes, **kwargs):
        ...
        nodes: list
          Atom instances.
        """
        CompoundAtom.__init__(self, nodes=nodes, **kwargs)
...
class CombinedAtom(CompoundAtom):
    """Atom to pass a dataset on to a set of nodes and combine there output.
    ...
    def _call(self, ds):
        ...
        from pyitab.dataset.dataset import hstack, vstack
