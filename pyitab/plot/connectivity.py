...
def plot_connectivity_linear(con, node_names, indices=None, n_lines=None,
                                     node_angles=None, node_width=None,
                                     node_size=None, node_thresh=None, con_thresh=None,
                                     node_colors=None, facecolor='black',
                                     textcolor='white', node_edgecolor='black',
                                     linewidth=1.1, colormap='magma', vmin=None,
                                     vmax=None, colorbar=True, title=None,
                                     colorbar_size=0.2, colorbar_pos=(-0.3, 0.1),
                                     fontsize_title=12, fontsize_names=7,
                                     fontsize_colorbar=15, padding=6.,
                                     fig=None, subplot=111, interactive=False):
    ...
    if node_size is None:
        node_size = minmax_scale(con.sum(0)) * 550 + 50
        #node_size = (con.sum(0) / con.sum(0).max())*400
    ...
    # Make figure background the same colors as axes
    if fig is None:
        fig = plt.figure(figsize=(18, 15), facecolor=facecolor)
    
    
    # Use a polar axes
    if not isinstance(subplot, tuple):
        subplot = (subplot,)
    axes = plt.subplot(*subplot, polar=False, facecolor=facecolor)
    ...
    for pos, (i, j) in enumerate(zip(indices[0], indices[1])):
        ...
        if np.abs(con[pos]) <= con_thresh:
            ...
            colormap = plt.get_cmap(str_cmap)
            alpha=0.8
            mult = np.abs(con[pos])/con.max() * 30

        color = colormap(con_val_scaled[pos])

        # Actual line
        patch = m_patches.PathPatch(path, fill=False, edgecolor=color,
                                    linewidth=10*linewidth[pos], alpha=alpha, zorder=0)
        axes.add_patch(patch)
...
def plot_connectivity_multilinear(con, node_names, indices=None, n_lines=None,
                                     node_angles=None, node_width=None,
                                     node_size=None, node_thresh=None, con_thresh=None,
                                     node_colors=None, facecolor='black',
                                     textcolor='white', node_edgecolor='black',
    ...
    node_angles = np.linspace(0, n_nodes, n_nodes, endpoint=False)
    ...
    if node_size is None:
        node_size = minmax_scale(con.sum(0)) * 800 + 150
        #node_size = (con.sum(0) / con.sum(0).max())*400
