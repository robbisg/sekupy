...
def cross_validate(estimator, X, y=None, *, groups=None, scoring=None, cv=None,
    ...
    return_decisions : boolean, default False
        Whether to return the estimators decisions.
    ...
    if return_splits:
        ret['splits'] = results["splits"]
    
    if return_predictions:
        ret['predictions'] = results["predictions"]

    if return_decisions:
        ret['decisions'] = results["decisions"]
    ...
    for name in test_scores_dict:
        ret['test_%s' % name] = np.array(test_scores_dict[name])
        if return_train_score:
            key = 'train_%s' % name
...
def _fit_and_score(estimator, X, y, scorer, train, test, verbose,
                   ...
                   return_splits=False, return_predictions=False, 
    ...
    if return_splits:
        result['splits'] = {'train': train, 'test': test}
    if return_predictions:
