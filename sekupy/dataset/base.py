...
from sekupy.dataset.dataset import AttrDataset, _expand_attribute
from sekupy.dataset.mappers import ChainMapper, FlattenMapper
from sekupy.dataset.selector import StaticFeatureSelection, mask_mapper
from sekupy.dataset.utils import table2string, is_sequence_type
...
class Dataset(AttrDataset):
    ...
    def select(self, sadict=None, fadict=None, strict=True):
        """Select samples/features given dictionaries describing selection.
    ...
    def __getitem__(self, args):
        ...
        if len(args) > 1 and 'mapper' in ds.a:
            ...
            # sekupy.dataset.base.Dataset has no clue about mappers and should
    ...
    def find_collection(self, attr):
        ...
        if attr in self.sa:
            col = self.sa
            if attr in self.fa or attr in self.a:
                logger.warning("An attribute with name '%s' is also present "
            ...
            if attr in self.a:
                logger.warning("An attribute with name '%s' is also present "
                               ...
                               "that you got the right one (see ``col`` argument).", attr)
        elif attr in self.a:
            col = self.a
...
def preprocessed_dataset(
    ...
    """Load and preprocess data into a dataset.
